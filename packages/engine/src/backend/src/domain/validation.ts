...
export interface WorldValidationResult {
  ...
  readonly issues: ReadonlyArray<WorldValidationIssue>;
}
...
function validateLightSchedule(
  ...
  const grid = 0.25;
  const onMod = Math.abs(schedule.onHours / grid - Math.round(schedule.onHours / grid));
  const offMod = Math.abs(schedule.offHours / grid - Math.round(schedule.offHours / grid));
  ...
  company.structures.forEach((structure, structureIndex) => {
    ...
    if (!isValidArea(structure.floorArea_m2)) {
      issues.push({
        ...
        message: `structure floor area must be a multiple of ${AREA_QUANTUM_M2}`
      });
    }
    ...
    structure.rooms.forEach((room, roomIndex) => {
      const roomPath = `${structurePath}.rooms[${roomIndex}]`;

      if (!isValidArea(room.floorArea_m2)) {
        issues.push({
          path: `${roomPath}.floorArea_m2`,
          message: `room floor area must be a multiple of ${AREA_QUANTUM_M2}`
        });
      }
      ...
      room.devices.forEach((device, deviceIndex) => {
        validateDevice(device, 'room', `${roomPath}.devices[${deviceIndex}]`, issues);
      });
      ...
      room.zones.forEach((zone, zoneIndex) => {
        ...
        if (!isValidArea(zone.floorArea_m2)) {
          issues.push({
            ...
            message: `zone floor area must be a multiple of ${AREA_QUANTUM_M2}`
          });
        }
        ...
        if (zone.photoperiodPhase !== 'vegetative' && zone.photoperiodPhase !== 'flowering') {
          issues.push({
        ...
        zone.devices.forEach((device, deviceIndex) => {
          validateDevice(device, 'zone', `${zonePath}.devices[${deviceIndex}]`, issues);
        });
