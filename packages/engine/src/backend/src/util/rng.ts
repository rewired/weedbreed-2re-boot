 ...
 */
/* eslint-disable @typescript-eslint/no-magic-numbers -- RNG seeding constants require fixed literal parameters */
...
function xmur3(str: string): () => number {
  ...
  for (let i = 0; i < str.length; i += 1) {
    h = Math.imul(h ^ str.charCodeAt(i), XMUR3_MULTIPLIER_A);
