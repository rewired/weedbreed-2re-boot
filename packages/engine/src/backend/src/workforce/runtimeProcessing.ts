...
import type { DeviceTelemetryEvent } from './telemetry/workforceEmit.ts';

/* eslint-disable @typescript-eslint/no-magic-numbers -- Workforce KPIs use fixed percentile thresholds */
export function isZoneQuarantined(world: SimulationWorld, zoneId: Zone['id'], currentTick: number): boolean {
...
export function ensureWorkforceState(state: WorkforceState | undefined, simDay: number): WorkforceState {
  if (state) {
    return state;
  }
...
export function appendCultivationTasks(
...
export function applyMaintenanceRuntime(
  ctx: EngineRunContext,
  world: SimulationWorld,
  ...
  let nextQueue: readonly WorkforceTaskInstance[] = taskQueue;
...
export function resolveStructureMaps(structures: readonly Structure[]): {
...
export function processHrAssignIntents({
  world,
  intents,
  employees,
  simTimeHours,
...
export function processManualTaskIntents({
  world,
  taskQueue,
  pestIntents,
  maintenanceIntents,
