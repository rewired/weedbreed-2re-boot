...
- Psychrometric helper tests (`packages/engine/tests/unit/shared/psychro/psychro.test.ts`) use `fast-check` to assert `computeVpd_kPa` stays finite and ≥0 for RH ∈ [0,100]; helper remains test-only until [docs/tasks/0009-psychrometric-wiring-plan.md](docs/tasks/0009-psychrometric-wiring-plan.md) promotes it into the pipeline and the pinned `psychrolib@1.1.x` dependency receives a maintained v2 release.
